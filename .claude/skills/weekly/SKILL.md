---
name: weekly
description: End-of-week Your Project narrative — what happened across the week, plus an exhaustive sweep of every problem/confusion/error in the week's Claude Code session transcripts, each with a concrete fix. Same detail bar as /daily but over the whole ISO week (catches what daily missed, plus a living-docs drift check). Workflow-fixable problems become proposed diffs (PROPOSED, not auto-applied; the user greenlights); every other problem is logged with a suggested action so nothing is dropped.
---
...
- **Claude Code session transcripts** under `~/.claude/projects/-home-your-username-your-project/*.jsonl` and `~/.claude/projects/-home-your-username-your-project--claude-worktrees-*/*.jsonl` — filter to files modified in the ISO week window
...
— `~/your-project/`). One file per ISO week. Use Python's
...
This is the auto-drafted summary the user will edit down.>
...
dropped handoffs, anything the user fixed by hand. One bullet each: what
...
- **User corrections** — "no", "don't", "stop", "wrong", "not what I meant", or the user significantly rewriting / redoing an artifact.
...
- **Dropped handoffs / manual fixes** — info lost between agents, or anything the user had to do by hand.
...
**Forbidden targets**:
- Hooks in `.claude/settings.json` — surface in `## My thoughts` for the user to wire via `/update-config`.
...
number. Order both sections by severity (the user's corrections / blockers
...
the user reads the proposals, replies with e.g. "do 1, 3" or "do all" or "skip 2 — let's discuss". The handling assistant spawns `workflow-improver` with the specific proposals. `workflow-improver` makes edits, runs `scripts/workflow_lint.py`, commits with message `workflow: apply weekly-proposed edits 1,3 (YYYY-Www)`.
