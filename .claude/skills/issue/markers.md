...
- `epm` = "experiment-pipeline manager" namespace (shared prefix, keeps our
...
| `plan` | skill (via adversarial-planner) | Step 2 | Goal, method delta, reproducibility card, success/kill criteria, GPU-hr estimate, compute spec. |
...
| `hf-gate-pending` | skill | Step 6a | Records that a gated-model auto-acceptance was triggered (model id, gate status, retry plan if rejected). |
| `pod-pending` | skill | Step 6b | Compute provisioning error, retry instructions. |
| `preflight` | skill | Step 6c | Full `--json` preflight report (resumed targets only). |
| `launch` | skill | Step 6d | Worktree, branch, PR, target identifier, PID, log path, code-review verdict, results-store URL (best-effort). |
...
| `results` | specialist | end of run | Eval JSON paths, filled reproducibility card, results-store URL, artifact-store path, commit hash, GPU-hours used, deviations, hot-fix log. For `type:infra` paths the implementer's completion report. **MUST include a `## Sample outputs` section with `### Condition: <name>` H3 subheadings and ≥3 randomly-sampled (input, prompt, response) triplets PER CONDITION, formatted as fenced markdown blocks. Verifier check #11 enforces.** |
...
| `pod-terminated` | skill | Step 10c | User opted to terminate the issue's compute target; records the target identifier and final volume disposition. |
| `pod-kept-stopped` | skill | Step 10c | User declined to terminate the target; it remains stopped and parked indefinitely. Records the target so resume / future-cleanup logic can find it. |
...
- Must ask: dataset changes, eval metric changes, compute spec changes

### Command to reproduce
```
nohup python scripts/train.py <args> seed=42 > <log-path>/issue-42.log 2>&1 &
```
<!-- /epm:plan -->
```
...
- Baseline comparison uses an earlier issue's results that ran under a
  different compute allocation. Should re-run baseline or qualify.
...
**Integration tests:** skipped (no compute target assigned)
