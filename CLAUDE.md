...
- **Reuse existing trained artifacts when fit-for-purpose — never reuse a wrong one.** The same default extends to TRAINED ARTIFACTS already on HF: LoRA adapters / merged checkpoints, training-mix JSONLs + raw-completion buckets, and `eval_results/` JSONs from prior tasks — before retraining or regenerating, the planner searches what already exists and reuses it when it fits the new Goal (canonical worked example: #532 reuses #474's loc-arm epoch-1 marker adapters instead of retraining 16 sources). Reuse is conditional on the POSITIVE fitness check **(a)-(g)** — recipe match grounded on the artifact's own `adapter_config.json` (#545), valid measurement regime, required cells present, no broken single-variable-change / measurement validity, HF resolution via `huggingface_hub.list_repo_files`, content identity / sha-pinning (#600), and the rsLoRA application-scaling parity probe (#601) — full checklist: `.claude/rules/artifact-reuse.md`. Silently reusing a wrong / stale / saturated artifact confounds the result and is WORSE than retraining; any check that fails → retrain / regenerate, and say why in the plan. Enforcement is a 3-stage defense: `planner.md` step 5 (self-attested fitness check) → `consistency-checker` (independent reuse-smuggled-variable diff vs the parent recipe) → `critic.md` Methodology lens item 9 (REVISE); the reuse provenance is then carried into the clean-result `## Reproducibility` (`analyzer.md`) and audited by `clean-result-critic` Lens 5.
...
**Concurrent repo-root committers.** Many sessions commit to the shared repo root in parallel. Stage by explicit path only (never `git add -A`/`git add .`), retry once on an `index.lock` collision, and on a rejected push `git pull --rebase=merges --autostash` once and re-push. The rebase mode is load-bearing: plain `--rebase` FLATTENS AWAY any not-yet-pushed merge commit sitting on the shared root — a concurrent session's documented recovery pull silently dropped the same workflow-fix merge twice on 2026-06-12 — and a bare rebase without `--autostash` predictably fails on the always-dirty shared root. The repo config pins `pull.rebase=merges` + `rebase.autoStash=true` (shared `.git/config`, covers all worktrees) so even a bare `git pull` behaves safely. **Push IMMEDIATELY after committing a merge** — an unpushed merge commit on the shared root is at risk from every concurrent session's pull; for multi-file merges during heavy fleet activity, prefer a scratch worktree detached at `origin/main` (`git worktree add --detach <path> origin/main` — `main` itself is held by the repo root) + `git push origin HEAD:main`. Before batch-fixing parked task bodies, post a claim marker (`epm:progress` note naming the task IDs) so a parallel session doesn't double-fix the same bodies.
...
**Authority split.** Live RunPod API is authoritative for pod state (existence, status, host, port, GPU, `created_at`); `scripts/pods.conf` is the SSH/MCP config source (refreshed from the live API by `provision`/`resume`, propagated outward by `pod.py config --sync`), and `scripts/pods_ephemeral.json` holds project metadata only. When SSH keeps failing on a port the live API no longer reports, pull the live host/port back into `pods.conf` with `pod.py config --refresh-from-api [<name>]` — full sync directions + when each applies: `.claude/rules/pod-config.md` (incident #488: a stale post-resume port spun an autonomous SSH polling loop for 13+ hours at $32/hr).

**Crons.** Three background automations keep the fleet clean — full predicates, env-var overrides, and incident history: `.claude/rules/background-automation.md`. (1) **Stale-pod audit** (09:37 daily + on `pod.py provision`): auto-terminates pods EXITED >24h; the owning task's `keep-running` tag exempts (reported `kept-exited`). (2) **Stale-worktree sweep** (09:47 daily, `worktree_audit.py --apply`): reaps idle auto-generated worktrees under `.claude/worktrees/` (live-process / active-issue / dirty worktrees kept; 6h grace, 1h under disk pressure; done-and-merged issue worktrees get orphaned-codex-pid + runtime-noise-rescue remediation); human-named worktrees are never touched, `issue-<N>-<suffix>` follow-up worktrees ARE in scope. (3) **Autonomous-session watcher** (every 10 min, `autonomous_session_watch.py`): crash-recovery respawn, pod-safety reconciliation, stalled-session detection, orphan-file sweep, a gate-push pass (Telegram phone push on gate-park/`blocked` transitions via the sibling-project `telegram_push.sh` channel, transition-deduped; status-transition-keyed title/self-report reconcile; force-stop of sessions flagged by `tick_triage.py`'s runaway counter), plus a reconcile pass that AUTO-STOPS issue-mapped sessions whose task is parked/terminal (`awaiting_promotion`/`completed`/`archived`, no live follow-up, idle >~2h, no running pod, no `keep-running` tag), a zombie-wrapper pass that auto-stops wrapper sessions with no inner Claude process ≥2h, and an idle-unmapped pass that auto-stops UNMAPPED project-cwd sessions whose transcript has been idle ≥12h (never a wrapper holding a live user TTY; a missing idleness signal fails toward keep; the 2026-06-12 VM-lag incident: 25 such sessions held ~23 GB RSS); sessions whose task is at any active status, `blocked`, or `plan_pending`, the PM session, and non-project sessions are never auto-stopped.
...
**Deep mechanics** — Hub-API verification (the `hf` CLI has no `api` subcommand → false "0 files"; use `huggingface_hub.list_repo_files`), the `EPM_SKIP_INLINE_CHECKPOINT_UPLOAD` inline-upload fence, the **delete-after-eval adapter-persist recipe** (#404/#458), and the **HF storage-quota-403 recovery** (#552/#541) — live in `.claude/rules/upload-policy.md` (loads when you touch training / hub / sweep code).
